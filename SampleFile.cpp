...
template<typename Child>
template<typename T>
auto SimDriver<Child>::update(double world_time, double time_step, int i)
-> decltype(static_cast<T *>(this)->onUpdate(world_time, time_step), void())
{
...
template<typename Child>
template<typename T>
auto SimDriver<Child>::update(double world_time, double time_step, long l) -> decltype(void())
{
    static_assert(std::is_same<T, Child>::value, "");
}
