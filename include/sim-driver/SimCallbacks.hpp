...
private:
    ...
    auto mouseButtonCallback(
        T& callbacks, const SimCallbacks<T>& parent, GLFWwindow* window, int button, int action, int mods, int i)
    ...
    auto keyCallback(T& callbacks,
                     ...
                     int action,
                     int mods,
                     int i) -> decltype(callbacks.keyCallback(window, key, scancode, action, mods, parent), void());
    template <typename T>
    auto
    cursorPosCallback(T& callbacks, const SimCallbacks<T>& parent, GLFWwindow* window, double xpos, double ypos, int i)
        -> decltype(callbacks.cursorPosCallback(window, xpos, ypos, parent), void());
    template <typename T>
    auto scrollCallback(
        T& callbacks, const SimCallbacks<T>& parent, GLFWwindow* window, double xoffset, double yoffset, int i)
        -> decltype(callbacks.scrollCallback(window, xoffset, yoffset, parent), void());
    template <typename T>
    auto charCallback(T& callbacks, const SimCallbacks<T>& parent, GLFWwindow* window, unsigned codepoint, int i)
    ...
    template <typename T>
    auto mouseButtonCallback(
        T& callbacks, const SimCallbacks<T>& parent, GLFWwindow* window, int button, int action, int mods, long l)
        -> decltype(void());
    template <typename T>
    auto keyCallback(T& callbacks,
                     const SimCallbacks<T>& parent,
    ...
    template <typename T>
    auto
    cursorPosCallback(T& callbacks, const SimCallbacks<T>& parent, GLFWwindow* window, double xpos, double ypos, long l)
        -> decltype(void());
    template <typename T>
    auto scrollCallback(
        T& callbacks, const SimCallbacks<T>& parent, GLFWwindow* window, double xoffset, double yoffset, long l)
        -> decltype(void());
    template <typename T>
    auto charCallback(T& callbacks, const SimCallbacks<T>& parent, GLFWwindow* window, unsigned codepoint, long l)
        -> decltype(void());
};
...
auto SimCallbacks<C>::windowFocusCallback(
    T& callbacks, const SimCallbacks<T>& parent, GLFWwindow* window, int focus, int)
    -> decltype(callbacks.windowFocusCallback(window, focus, parent), void())
{
...
auto SimCallbacks<C>::mouseButtonCallback(
    T& callbacks, const SimCallbacks<T>& parent, GLFWwindow* window, int button, int action, int mods, int)
    -> decltype(callbacks.mouseButtonCallback(window, button, action, mods, parent), void())
{
    static_assert(std::is_same<T, C>::value, "");
...
auto SimCallbacks<C>::keyCallback(
    T& callbacks, const SimCallbacks<T>& parent, GLFWwindow* window, int key, int scancode, int action, int mods, int)
...
auto SimCallbacks<C>::cursorPosCallback(
    T& callbacks, const SimCallbacks<T>& parent, GLFWwindow* window, double xpos, double ypos, int)
    -> decltype(callbacks.cursorPosCallback(window, xpos, ypos, parent), void())
{
    static_assert(std::is_same<T, C>::value, "");
...
auto SimCallbacks<C>::scrollCallback(
    T& callbacks, const SimCallbacks<T>& parent, GLFWwindow* window, double xoffset, double yoffset, int)
    -> decltype(callbacks.scrollCallback(window, xoffset, yoffset, parent), void())
{
    static_assert(std::is_same<T, C>::value, "");
...
auto SimCallbacks<C>::charCallback(
    T& callbacks, const SimCallbacks<T>& parent, GLFWwindow* window, unsigned codepoint, int)
    -> decltype(callbacks.charCallback(window, codepoint, parent), void())
{
    static_assert(std::is_same<T, C>::value, "");
...
template <typename C>
template <typename T>
auto SimCallbacks<C>::keyCallback(T&, const SimCallbacks<T>&, GLFWwindow*, int, int, int, int, long) -> decltype(void())
{
}
template <typename C>
template <typename T>
...
template <typename C>
template <typename T>
auto SimCallbacks<C>::scrollCallback(T&, const SimCallbacks<T>&, GLFWwindow*, double, double, long) -> decltype(void())
{
}
template <typename C>
template <typename T>
